...
class FilterLines:
    ...
    def process(self, source0):
        ...
        (self.get_mat_info_size, self.get_mat_info_empty, self.get_mat_info_channels, self.get_mat_info_cols,
         self.get_mat_info_rows, self.get_mat_info_high_value) = self.__get_mat_info(self.__get_mat_info_input)
...
class ContourTracker:
    ...
    def find_closest_contour(self, contours, frame_size):
        ...
        if len(remainder) > 0:
            remainder = min(remainder, key=lambda target: fabs(target.average_x - frame_size[1] / 2))
        else:
            remainder = None
